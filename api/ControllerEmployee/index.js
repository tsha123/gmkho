        ...
        try {
            if (
                !(await helper.checkPermission(
                    '61e15741f8bf2521b16be201',
                    req.body._caller.id_employee_group
                ))
            )
                return res
                    .status(403)
                    .send(
                        'Thất bại! Bạn không có quyền truy cập chức năng này'
                    );
            ...
            if (
                validator.isDefine(req.query.id_employee_group) &&
                validator.ObjectId.isValid(req.query.id_employee_group)
            ) {
                query = {
                    ...query,
                    id_employee_group: req.query.id_employee_group,
                };
            }
            var limit = 10;
            var page = 0;
            ...
            for (let i = 0; i < data.length; i++) {
                const group = await ModelEmployeeGroup.findById(
                    data[i].id_employee_group
                );
    ...
    app.put(prefixApi, helper.authenToken, async (req, res) => {
        try {
            ...
            upload_image_branch(req, res, async (err) => {
                ...
                const dataEm = await ModelEmployee.findById(
                    req.body.id_employee
                );
                if (!dataEm)
                    return res
                        .status(400)
                        .send('Thất bại! Không tìm thấy nhân viên');
                const dataGroup = await ModelEmployeeGroup.findById(
                    dataEm.id_employee_group
                );
                if (!dataGroup)
                    return res
                        .status(400)
                        .send(
                            'Thất bại! Không tìm thấy chức danh của nhân viên'
                        );
                const dataGroupOfAdmin = await ModelEmployeeGroup.findById(
                    id_employee_group_admin
                );
                if (!dataGroupOfAdmin)
                    return res
                        .status(400)
                        .send('Thất bại! Không tìm thấy chức danh của bạn');
                if (dataGroupOfAdmin.employee_level > dataGroup.employee_level)
                    return res
                        .status(400)
                        .send(
                            'Thất bại! Bạn không có quyền sửa thông tin của nhân viên cao hơn chức danh hiện tại của bạn'
                        );
                ...
                if (
                    validator.isDefine(req.body.password) &&
                    req.body.password.length > 0
                )
                    newValue = { ...newValue, password: req.body.password };
                if (
                    validator.isDefine(req.body.employee_fullname) &&
                    req.body.employee_fullname.length > 0
                )
                    newValue = {
                        ...newValue,
                        employee_fullname: req.body.employee_fullname,
                    };
                if (validator.isDefine(req.body.employee_address))
                    newValue = {
                        ...newValue,
                ...
                if (validator.isDefine(req.body.id_employee_group))
                    newValue = {
                        ...newValue,
                        id_employee_group: req.body.id_employee_group,
                    };
                ...
                if (validator.isDefine(req.body.employee_status))
                    newValue = {
                        ...newValue,
                        employee_status: req.body.employee_status,
                    };
                if (validator.isDefine(req.body.employee_phone))
                    newValue = {
                        ...newValue,
                        employee_phone: req.body.employee_phone,
                    };
                if (validator.isDefine(req.body.id_branch))
                    newValue = { ...newValue, id_branch: req.body.id_branch };
                try {
                    const updateNew = await ModelEmployee.findByIdAndUpdate(
                        dataEm._id,
                        newValue
                    );
                    if (!req.file) {
                        await validator.removeFile(
                            validator.URL_IMAGE_EMPLOYEE +
                                `/${dataEm.employee_image}`
                        );
                    }
            ...
            upload_image_branch(req, res, async (err) => {
                ...
                if (dataGroupOfAdmin.employee_level > dataGroup.employee_level)
                    return res
                        .status(400)
                        .send(
                            'Thất bại! Bạn không có quyền tạo tài khoản có chức danh cao hơn bạn'
                        );
                ...
                // if (validator.isDefine(req.body.employee_bank_number))
                //     newValue = {
                //         ...newValue,
                //         employee_bank_number: req.body.employee_bank_number,
                //     };
                // if (validator.isDefine(req.body.employee_bank_name))
                //     newValue = {
                //         ...newValue,
                ...
                // if (validator.isDefine(req.body.employee_level))
                //     newValue = {
                //         ...newValue,
                //         employee_level: req.body.employee_level,
                //     };
                // if (validator.isDefine(req.body.employee_salary))
                //     newValue = {
                //         ...newValue,
                //         employee_salary: validator.tryParseInt(
                //             req.body.employee_salary
                //         ),
                //     };
                // if (validator.isDefine(req.body.employee_salary_duty))
                //     newValue = {
                //         ...newValue,
                //         employee_salary_duty: validator.tryParseInt(
                //             req.body.employee_salary_duty
                //         ),
                //     };
                // if (validator.isDefine(req.body.employee_lunch_allowance))
                //     newValue = {
                //         ...newValue,
                //         employee_lunch_allowance: validator.tryParseInt(
                //             req.body.employee_lunch_allowance
                //         ),
                //     };
                // if (validator.isDefine(req.body.employee_revenue_percent))
                //     newValue = {
                //         ...newValue,
                //         employee_revenue_percent: parseFloat(
                //             req.body.employee_revenue_percent
                //         ),
                //     };
                if (validator.isDefine(req.body.employee_status))
                    newValue = { ...newValue, employee_status: true };
                if (validator.isDefine(req.body.employee_phone))
                    newValue = {
                        ...newValue,
                        employee_phone: req.body.employee_phone,
...
export const insert_group = async (app) => {
    app.post(prefixApi + '/add-group', helper.authenToken, async (req, res) => {
        try {
            ...
            if (!validator.isDefine(group_name))
                return res
                    .status(400)
                    .send(`Thất bại! Tên chức danh không được để trống.`);
            if (!validator.ObjectId.isValid(id_supper))
                return res
                    .status(400)
                    .send(`Thất bại! Hãy chọn nhóm nhân viên.`);
