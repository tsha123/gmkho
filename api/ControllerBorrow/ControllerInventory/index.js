...
export const checkPermission = async (app) => {
    app.get(prefixApi +"/checkPermission", helper.authenToken, async (req, res) => {
        ...
        const warehouses = await ModelWarehouse.find()
                    ...
                    $cond:
                    {
                        ...
                        then: {
                            $cond: {
                                ...
                                else: { $multiply: [{ $subtract: ["$import_form_product.product_import_price", { $multiply: [{ $divide: ["$import_form_product.product_import_price", 100] }, 0] }] }, "$import_form_product.product_quantity"] }
                            }
                        },
                        else: 0
                    }
