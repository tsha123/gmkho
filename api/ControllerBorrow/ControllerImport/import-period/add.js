        ...
        try{
            ...
            for (let i = 0; i < arrProductFormImport.length; i++){
                for(let j = 0;j<arrProductFormImport[i].product_quantity;j++){
                    ...
                    await new ModelHistoryProduct({
                        product_id:objectProduct._id,
                        content: `Nhập hàng (nhập tiếp) ${dataImport.import_form_type} bởi nhân viên ${employee_name} vào kho ${dataWarehouse.warehouse_name}, mã phiếu nhập : ${dataImport._id}, giá nhập: ${arrProductFormImport[i].product_import_price}`
                    }).save()
                }
            }
            try {
