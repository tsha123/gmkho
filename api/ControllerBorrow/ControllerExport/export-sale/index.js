...
export const update = async (app) => {
    app.put(prefixApi, helper.authenToken, async (req, res) => {
        try {
            ...
            const data_warehouse = await ModelWarehouse.findById(dataExport.id_warehouse)

            const dataDebt = await ModelDebt.findOne({ id_form: dataExport._id })
            if (!dataDebt && dataExport.export_source == "app") {

            }
            else if (!dataDebt) {

                return res.status(400).send("Thất bại! Không tìm thầy công nợ cũ")
            }
            ...
            for (let i = 0; i < arrProduct.length; i++) {
                await new ModelHistoryProduct({
                    ...
                    content: `Sửa phiếu xuất, mã phiếu :${dataExport._id} bởi nhân viên ${employee_name} bảo hành :${arrProduct[i].product_warranty} giá xuất: ${arrProduct[i].product_export_price} giá vốn: ${arrProduct[i].product_import_price}`
                }).save()
            }
            ...
            if (dataReceive) {
                await ModelReceive.findByIdAndUpdate(dataReceive._id, {
                ...
                if (receive_form_money > 0 || is_payment_zero) {
                    const insertReceive = new ModelReceive({
                        ...
                        id_branch: data_warehouse.id_branch,
            ...
            if (dataDebt) {
                ...
                await new ModelDebt({
                    ...
                    id_branch: data_warehouse.id_branch,
                    id_employee: id_employee,
                    debt_money_receive: total + dataExport.money_point + dataExport.money_voucher_code,
                    debt_money_export: total,
            ...
            const updateExport = await ModelExportForm.findByIdAndUpdate(dataExport._id, { // 
                ...
                id_user: dataUser._id,
                export_status_by_app: receive_form_money > 0 ? true : export_form_note.export_status_by_app
            })
    ...
    app.get(prefixApi + "/print", helper.authenToken, async (req, res) => {
        try {
            ...
            if (dataReceive) {
                ...
                if (!dataExport.export_status_by_app) {
                    dataExport.receive_money = dataExport.payment_temp
                    const dataFund = await ModelFundBook.findById(dataExport.id_fundbook_temp)
                    if (dataFund) dataExport.fundbook_name = dataFund.fundbook_name
                }
            }
    ...
    app.delete(prefixApi + "/product", helper.authenToken, async (req, res) => {
        try {
            ...
            await new ModelHistoryProduct({
                ...
                content: `Xóa khỏi phiếu xuất, mã phiếu :${dataExport._id} bởi nhân viên ${employee_name}`
            }).save()
...
export const update_employee = async (app) => {
    app.put(`${prefixApi}/employee`, helper.authenToken, async (req, res) => {
        try {
            ...
            const dataExport = await ModelExportForm.findById(id_export)
            if (!dataExport) return res.status(400).send("Thất bại! Không tìm thấy phiếu xuất")
            const dataDebt = await ModelDebt.findOne({ $and: [{ debt_type: "export" }, { id_form: dataExport._id }] })
            if (!dataDebt) return res.status(400).send(`Thất bại! Không thấy công nợ`)
            ...
            for (let i = 0; i < arrProduct.length; i++) {
                for (let j = 0; j < dataExport.export_form_product.length; j++) {
                    if (arrProduct[i].id_product.toString() == dataExport.export_form_product[j].id_product.toString()) {
                        dataExport.export_form_product[j].id_employee = !validator.isDefine(arrProduct[i].id_employee) ? null : arrProduct[i].id_employee
                        dataExport.export_form_product[j].id_employee_setting = validator.isDefine(arrProduct[i].id_employee) ? null : arrProduct[i].id_employee_setting
                        dataExport.export_form_product[j].product_warranty = arrProduct[i].product_warranty
                        dataExport.export_form_product[j].product_import_price = arrProduct[i].product_import_price
                        dataExport.export_form_product[j].product_export_price = arrProduct[i].product_export_price
                    }

                    await new ModelHistoryProduct({
                        product_id: arrProduct[i].id_product,
                        content: `Sửa phiếu xuất, sửa nhân viên, mã phiếu :${dataExport._id} bởi nhân viên ${employee_name}, bảo hành :${arrProduct[i].product_warranty} giá xuất: ${arrProduct[i].product_export_price} giá vốn: ${arrProduct[i].product_import_price}`
                    }).save()
                }
            }
            ...
            const total_money = validator.calculateMoneyExport(dataExport.export_form_product) - dataExport.money_voucher_code

            const receive_form_money = validator.tryParseInt(req.body.receive_form_money)
            const dataReceive = await ModelReceive.findOne({ id_form: dataExport._id })
            if (dataReceive) {
                await ModelReceive.findByIdAndUpdate(dataReceive._id, {
                    receive_money: receive_form_money,
            ...
            await ModelDebt.updateOne({ _id: dataDebt._id }, {
                $set: {
                    debt_money_export: total_money,
                    debt_money_receive: receive_form_money
